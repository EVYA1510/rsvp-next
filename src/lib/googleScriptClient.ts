...
export async function submitToGoogleScript(
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      console.log(`Attempting to submit to Google Script (attempt ${attempt}/${retries})`);
      ...
      if (attempt === retries) {
        throw new Error(`Failed to submit to Google Script after ${retries} attempts: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
...
export function getGoogleScriptConfig(): {
  ...
  return {
    url,
    isConfigured: !!url && url !== "https://script.google.com/macros/s/YOUR_SCRIPT_ID/exec",
  };
}
